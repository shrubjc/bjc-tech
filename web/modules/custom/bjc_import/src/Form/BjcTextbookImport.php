...
/**
 * @file
 * Contains \Drupal\bjc_import\Form\BjcTextbookImport.
 */

namespace Drupal\bjc_import\Form;
...
/**
 * Implements the BjcImport form controller.
 *
 * @see \Drupal\Core\Form\FormBase
 */
class BjcTextbookImport extends FormBase {

  /**
   * {@inheritdoc}
   */
  public function getFormId() {
    return 'bjc_import_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    ...
    $context = array('context' => BJC_IMPORT_TRANSLATE_CONTEXT);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($type != '' && !empty($csv_file)) {
      ...
      while (($import_row = fgetcsv($fp, BJC_DATA_IMPORT_UPDATE_CSV_ROW_LENGTH, ",")) !== FALSE) {
        $data[] = $import_row;
      }
      if (!empty($data)) {
        array_shift($data);
      ...
      if ($type == TEXTBOOKS_CONTENT_TYPE) {
        $operations = [];
        foreach (array_chunk($data, BJC_DATA_IMPORT_BATCH_SIZE) as $smaller_batch_data) {
